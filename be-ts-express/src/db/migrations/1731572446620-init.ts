...
export class Init1731572446620 implements MigrationInterface {
    name = 'Init1731572446620'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`search_summary_fields\` (\`id\` int NOT NULL AUTO_INCREMENT, \`searchType\` varchar(255) NOT NULL, \`valueNum\` float NULL, \`valueStr\` varchar(255) NULL, \`searchId\` int NOT NULL, \`searchIdId\` int NULL, \`searchIdSearchTerm\` varchar(255) NULL, PRIMARY KEY (\`id\`, \`searchType\`, \`searchId\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`search_summary_fields\` ADD CONSTRAINT \`FK_5dbb5376ad3ac75e23982459c39\` FOREIGN KEY (\`searchIdId\`, \`searchIdSearchTerm\`) REFERENCES \`search\`(\`id\`,\`searchTerm\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`search_summary_fields\` DROP FOREIGN KEY \`FK_5dbb5376ad3ac75e23982459c39\``);
