...
export class Init1736456821802 implements MigrationInterface {
    name = 'Init1736456821802'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`search\` (\`id\` int NOT NULL AUTO_INCREMENT, \`searchTerm\` varchar(255) NOT NULL, \`url\` varchar(255) NOT NULL, \`categoryId\` varchar(255) NULL, \`metadataUpdatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), UNIQUE INDEX \`IDX_e3a4ab7a296023d1fbe8ac19d7\` (\`searchTerm\`), PRIMARY KEY (\`id\`, \`searchTerm\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`search_position\` (\`position\` int NOT NULL, \`metadataUpdatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`searchId\` int NULL, \`searchSearchTerm\` varchar(255) NULL, \`productId\` varchar(255) NULL, UNIQUE INDEX \`IDX_eb8fc9945d5ed4f0089ff3ff38\` (\`searchId\`, \`searchSearchTerm\`, \`position\`, \`productId\`), UNIQUE INDEX \`REL_c42dba2b59ca7c73d645a173a8\` (\`productId\`), PRIMARY KEY (\`position\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`search_position\` ADD CONSTRAINT \`FK_5ef9f5ece4a8777c231eb61acaf\` FOREIGN KEY (\`searchId\`, \`searchSearchTerm\`) REFERENCES \`search\`(\`id\`,\`searchTerm\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`search_position\` DROP FOREIGN KEY \`FK_5ef9f5ece4a8777c231eb61acaf\``);
        ...
        await queryRunner.query(`DROP INDEX \`IDX_eb8fc9945d5ed4f0089ff3ff38\` ON \`search_position\``);
