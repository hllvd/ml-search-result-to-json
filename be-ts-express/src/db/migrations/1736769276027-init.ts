...
export class Init1736769276027 implements MigrationInterface {
    name = 'Init1736769276027'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`jobs\` (\`id\` varchar(36) NOT NULL, \`description\` varchar(128) NOT NULL, \`status\` int NOT NULL, \`metadataUpdatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`job_rows\` (\`id\` varchar(36) NOT NULL, \`jobType\` tinyint NOT NULL, \`jobStatus\` tinyint NOT NULL, \`jobPriority\` tinyint NOT NULL, \`payload\` varchar(255) NOT NULL, \`metadataUpdatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`jobsId\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`job_rows\` ADD CONSTRAINT \`FK_07f62b9e865ac34b91db42139f9\` FOREIGN KEY (\`jobsId\`) REFERENCES \`jobs\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`job_rows\` DROP FOREIGN KEY \`FK_07f62b9e865ac34b91db42139f9\``);
        ...
        await queryRunner.query(`DROP TABLE \`job_rows\``);
